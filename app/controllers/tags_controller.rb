class TagsController < ApplicationController
  def index
    ...
    @genre_tags = GenreTag.joins(:posts).group(:name).select("name, COUNT(*) as count").order("count DESC").limit(3)
    @taste_tags = TasteTag.joins(:posts).group(:name).select("name, COUNT(*) as count").order("count DESC").limit(3)
    @outher_tags = OutherTag.joins(:posts).group(:name).select("name, COUNT(*) as count").order("count DESC").limit(3)
  end
