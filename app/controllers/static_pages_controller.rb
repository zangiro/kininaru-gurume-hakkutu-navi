class StaticPagesController < ApplicationController
  def top
    if logged_in? && current_user.view_history_posts.present?
      @current_user_view_histories = current_user.view_history_posts.order("view_histories.created_at DESC").limit(50)
      ...
      @all_user_view_histories = Post.joins(:view_histories).order("view_histories.created_at DESC").limit(50)
