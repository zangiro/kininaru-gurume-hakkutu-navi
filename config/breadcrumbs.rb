...
crumb :post_show do |post, area_tags, genre_tags, taste_tags, outher_tags, user, word, post_path|
  ...
  if post_path == POST_INDEX_POST_PATH
    link title
    parent :post_index, user
  elsif post_path == SEARCH_BY_FORM_POST_PATH
    link title
    parent :search_by_form, word
  elsif post_path == SEARCH_INDEX_POST_PATH
    link title
    parent :search_index, area_tags, genre_tags, taste_tags, outher_tags
  elsif post_path == LIKE_INDEX_POST_PATH
    link title
    parent :like_index, user
  elsif post_path == VIEW_HISTORY_INDEX_POST_PATH
    link title
